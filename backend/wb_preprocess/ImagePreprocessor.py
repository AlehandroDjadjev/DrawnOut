...
def process_images(indir):
    """Batch-process all images in *indir* using process_one()."""
    import shutil
    if OUT_DIR.exists():
    ...
    imgs = sorted(
        [p for p in Path(indir).glob("*") if p.suffix.lower() in (
            ".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff", ".webp"
        )],
        key=lambda p: p.name.lower(),
    )
    print(f"[INFO] found {len(imgs)} image(s) in {indir}.")
    if not imgs:
        return
    for p in imgs:
        process_one(p)
