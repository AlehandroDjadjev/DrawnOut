...
from ImageVectorizer import vectorize_images, vectorize_in_memory
from ImageVecOrganizer import organize_images

try:
    from ImagePreprocessor import process_images_in_memory as preprocess_images_in_memory
except ImportError:
    preprocess_images_in_memory = None

try:
    from ImageSkeletonizer import skeletonize_in_memory as skel_in_memory
except ImportError:
    skel_in_memory = None
...
def vectorize_image(request):
    ...
    Accepts multipart image upload, returns stroke JSON (strokes, width, height, vector_format).
    """
    if preprocess_images_in_memory is None or skel_in_memory is None:
        return JsonResponse(
            {"ok": False, "error": "Vectorizer modules unavailable"},
            status=500,
        )
    ...
    try:
        ...
        preproc = preprocess_images_in_memory([text_item])
        skels = skel_in_memory(preproc)
        vectors = vectorize_in_memory(skels)
