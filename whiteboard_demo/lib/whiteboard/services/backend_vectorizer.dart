...
class BackendVectorizer {
  ...
  static Future<List<List<Offset>>> fetchTextStrokesAsPolylines({
    ...
    try {
      ...
      return _decodeTextStrokesToPolylines(
  ...
  static List<List<Offset>> _decodeTextStrokesToPolylines(
    ...
    final format =
        (data['vector_format'] as String?)?.toLowerCase() ?? 'bezier_cubic';
      ...
      for (final s in strokesJson) {
        ...
        for (final seg in segsJson) {
          if (seg is List && seg.length >= 8) {
            ...
            final steps = _recommendedStepsText(p0, c1, c2, p1);
  ...
  static List<List<Offset>> _decodeToPolylines(
    ...
    if (format == 'bezier_cubic') {
      for (final s in strokesJson) {
        ...
        final cleaned = _removeTinySteps(pts);
  ...
  /// Higher sampling for text strokes (matches visual_whiteboard stepsPerSegment=18).
  /// Font glyph curves need denser sampling; skip _removeTinySteps which over-decimates.
